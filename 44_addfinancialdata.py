#Updated Financial Logic for add feature accepts both csv and add feature as a input
...
result
{     "srno": 0,     "asset_class": [         "NKE"     ],     "Financial_Term": "Q1-2025",     "Total_Sales_m": 8050.4,     "Store_Operating_Expenses_m": 3400,     "Product_and_Distribution_Costs_m": 2526.9,     "Depreciation_and_Amortization_Expenses_m": 366,     "Selling__Marketing_m": 0,     "SM__of_Sales": 0,     "Research__Development_m": 0,     "RD__of_Sales": 0,     "General__Administrative_m": 525.8,     "Other_m": 101.7,     "Depreciation_m": 386.4,     "Cash_and_Cash_Equivalents_m": 4056.8,     "Longterm_Investments_m": 551.5,     "Longterm_Debt_m": 13586.3,     "Fully_diluted_shares_outstanding_m": 1176.6,     "user_id": "78292" }
...
if "request" in cache and cache["request"]:
    ...
    df = request_df  # Assign directly to df if only request input is present

# Check if cache["result"] is already a dictionary or needs to be parsed
elif "result" in cache and cache["result"]:
    if isinstance(cache["result"], str):
        result = json.loads(cache["result"])  # Parse the string into a dictionary
    else:
        result = cache["result"]  # Use it directly as it's already a dictionary
    result_df = pd.json_normalize(result)

    # Step 2: Handle list in 'asset_class' for manual input (explode to rows)
    result_df = result_df.explode('asset_class')

    # Step 3: Fill missing '_id' for manual input
    result_df['_id'] = result_df['srno'].apply(lambda x: "")


    df = result_df  # Assign directly to df if only result input is present

else:
    # If neither input is present
    df = pd.DataFrame()

# Proceed with further steps (e.g., year and quarter extraction, YOY calculations, etc.)
...
df['Percent_Change'] = np.where(
    ...
    (df['Quarterly_Operating_Cash_Flow_m'] / df['Sales_Current_Year']) - 1,
    0
)
